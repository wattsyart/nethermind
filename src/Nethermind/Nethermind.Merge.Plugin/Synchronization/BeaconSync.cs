...
namespace Nethermind.Merge.Plugin.Synchronization
{
    public class BeaconSync : IMergeSyncController, IBeaconSyncStrategy
    {
        ...
        public bool IsBeaconSyncHeadersFinished()
        {
            ...
            bool chainMerged =
                ((lowestInsertedBeaconHeader?.Number ?? 0) - 1) <=
                (_blockTree.BestSuggestedHeader?.Number ?? long.MaxValue) &&
            ...
            if (_logger.IsTrace)
                _logger.Trace(
                    $"IsBeaconSyncHeadersFinished: {finished}, BeaconPivotExists: {_beaconPivot.BeaconPivotExists()}, LowestInsertedBeaconHeaderNumber: {_blockTree.LowestInsertedBeaconHeader?.Number}, BeaconPivot: {_beaconPivot.PivotNumber}, BeaconPivotDestinationNumber: {_beaconPivot.PivotDestinationNumber}");
            return finished;
        }
